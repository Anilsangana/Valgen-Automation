    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            if (data.results.role && data.results.role.length > 0) {
                addResultSection(doc, 'Role Creation', data.results.role, (item: any) => {
            ...
            if (data.results.department && data.results.department.length > 0) {
                addResultSection(doc, 'Department Creation', data.results.department, (item: any) => {
                    const fields: Array<{ label: string; value: string }> = [
            ...
            if (data.results.user && data.results.user.length > 0) {
                addResultSection(doc, 'User Creation', data.results.user, (item: any) => {
                    const fields: Array<{ label: string; value: string }> = [
            ...
            if (data.results.deactivation && data.results.deactivation.length > 0) {
                addResultSection(doc, 'User Deactivation', data.results.deactivation, (item: any) => {
                    const fields: Array<{ label: string; value: string }> = [
                        { label: 'Username', value: item.username || 'N/A' },
                        { label: 'Status', value: item.status || 'N/A' },
            ...
            doc.fontSize(8)
                .fillColor('#a0aec0')
                .text(
                    'This is an automatically generated audit report for GCP compliance and record-keeping purposes.',
                    50,
                    pageHeight - 70,
