...
const dbPath = path.join(process.cwd(), 'data', 'valgenesis.db');
...
export async function getDb(): Promise<sqlite3.Database> {
  ...
  dbSingleton = await new Promise<sqlite3.Database>((resolve, reject) => {
...
async function initDb(db: sqlite3.Database): Promise<void> {
