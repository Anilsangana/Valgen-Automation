...
export async function createDepartment(
  ...
  for (const d of departments) {
    try {
      automationEvents.emit('log', `Creating department: ${d.name}`);
      ...
      // ===================== SUCCESS =====================
      automationEvents.emit('log', `Department created (popup=${popupAppeared}) for: ${d.name}`);
      ...
      automationEvents.emit('error', `Department creation failed for ${d.name}: ${String(err)}`);
      results.push({
        department: d.name,
        status: 'error',
        message: String(err)
