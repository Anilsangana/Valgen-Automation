...
export type GroupInput = {
    name: string;
    groupType?: 'Internal' | 'External' | 'Review and Approval';
    description?: string;
...
export async function createGroup(
    ...
    for (const grp of groups) {
        try {
            automationEvents.emit('log', `Creating group: ${grp.name}`);
            ...
            automationEvents.emit('log', '✓ Group form loaded with users ready');
            ...
            if (currentValue !== desiredValue) {
                automationEvents.emit('log', `→ Changing Group Type to: ${desiredType}`);
                await frame.locator('#ddlGroupType').selectOption(desiredValue);

                // Wait for page-level overlay AND iframe DevExpress list loaders
                await waitForOverlayGone(page, 20000);
                await waitForIframeLoaderGone(page, 20000);
                automationEvents.emit('log', `✓ Group type changed — user list reloaded`);
            } else {
                automationEvents.emit('log', `✓ Group type already set to: ${desiredType}`);
            }
            ...
            if (grp.selectAllUsers !== false) {
                try {
                    ...
                    if (selectAllVal === 'C') {
                        automationEvents.emit('log', '✓ All available users selected');
                    } else {
                        automationEvents.emit('log', `⚠️ Select All value: "${selectAllVal}" — may be empty list or already selected`);
                    }
                } catch (e) {
                    automationEvents.emit('log', `⚠️ Could not select all users: ${String(e)}`);
                }
            }

            // ===================== STAGE 3: SUBMIT =====================
            // Ensure page is stable before submitting
            await waitForOverlayGone(page, 10000);
            automationEvents.emit('log', '→ Clicking Submit...');
            ...
            if (popupAppeared === 'notification') {
                automationEvents.emit('log', '⚠️ Notification: "Select Users from Available list" — user list was empty');
            ...
            // ===================== SUCCESS =====================
            automationEvents.emit('log', `✓ Group created (popup=${popupAppeared}): ${grp.name}`);
            await frame.locator('#btnMessageOk').click().catch(() => { });
            ...
            automationEvents.emit('error', `Group creation failed for ${grp.name}: ${String(err)}`);
