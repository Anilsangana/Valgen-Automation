...
async function configureRolePermissions(frame, page, roleName) {
    ...
    if (successMessage) {
        browser_1.automationEvents.emit('log', 'Permissions saved successfully');
    }
    if (!successMessage) {
        browser_1.automationEvents.emit('log', 'Failed to assign Permissions');
    }
    await page.waitForTimeout(1000);
...
async function createRole(page, roles, options = {}) {
    const results = [];
    ...
    for (const r of roles) {
        try {
            browser_1.automationEvents.emit('log', `Creating role: ${r.roleName}`);
            // ===================== Navigation =====================
            ...
            browser_1.automationEvents.emit('error', `Role creation failed for ${r.roleName}: ${String(err)}`);
            results.push({
                role: r.roleName,
                status: 'error',
                message: String(err)
