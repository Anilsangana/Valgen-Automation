 ...
 */
async function createCategory(page, categories, options = {}) {
    const results = [];
    const system = new systemPage_1.SystemPage(page);
    const strategy = options.duplicateStrategy ?? 'skip';
    for (const cat of categories) {
        try {
            browser_1.automationEvents.emit('log', `Creating category: ${cat.name}`);
            // ===================== NAVIGATION =====================
            ...
            // ===================== SUCCESS =====================
            browser_1.automationEvents.emit('log', `Category created (popup=${popupAppeared}) for: ${cat.name}`);
            ...
            browser_1.automationEvents.emit('error', `Category creation failed for ${cat.name}: ${String(err)}`);
            results.push({
                category: cat.name,
                status: 'error',
                message: String(err)
